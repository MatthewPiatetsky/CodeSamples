...
public class PictureDao {
  private static final String LOAD_PICTURE_STMT = "SELECT " +
      "\"caption\", \"imgdata\", \"thumbdata\", \"size\", \"content_type\",\"email\" FROM Pictures WHERE \"picture_id\" = ?";

  private static final String SAVE_PICTURE_STMT = "INSERT INTO " +
      "Pictures (\"caption\", \"imgdata\", \"thumbdata\", \"size\", \"content_type\",\"email\") VALUES (?, ?, ?, ?, ?,?)";
  ...
  public Picture load(int id) {
	...
	public void save(Picture picture) {
		...
		try {
			...
			stmt.setString(6,picture.getOwnerEmail());
			stmt.executeUpdate();
